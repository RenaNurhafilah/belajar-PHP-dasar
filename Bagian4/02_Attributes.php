...
function validateNotBlank(ReflectionProperty $property, object $object): void {
    ...
    if(count($attributes) > 0) {
        if(!$property->isInitialized($object))
            throw new Exeption("Property $property->name is null");
...
function validateLength(ReflectionProperty $property, object $object): void {
    ...
    foreach ($attributes as $atribute) {
        ...
        if($valueLength < $length->min)
            throw new Exeption("Property $property->name is too short");
        if($valueLength < $length->max)
            throw new Exeption("Property $property->name is too long");
        
    }
}

$request = new LoginRequest();
$request->username = "rena123";
